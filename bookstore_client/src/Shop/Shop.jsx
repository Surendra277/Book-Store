...
const Shop = () => {
  ...
  return (
    <div className="flex relative mt-14">
      <aside className="w-1/6 fixed flex flex-col items-center bg-teal-200  shadow-lg p-4">
      ...
      <main className="w-3/4 lg:ml-80">
        ...
        <div className="mx-5  grid gap-8 lg:grid-cols-4 sm:grid-cols-2 md:grid-cols-3 grid-cols-1">
          {filteredBooks.map((book) => (
            <Card key={book._id} className="relative shadow-lg hover:shadow-xl transition-shadow duration-300">
              <BookCard book={book}/>
            </Card>
          ))}
        </div>
